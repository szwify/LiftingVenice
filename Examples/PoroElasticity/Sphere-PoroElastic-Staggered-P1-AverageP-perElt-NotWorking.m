...
% elasticity dof
dof_he=DOF_handle(mesh_fem,2,'Matrix');  % the dof_handle
% pressure dof
...
%%%%%% additional matrix for staggered coupling
%%%%%%%
%%%% pressure average on Triangular Element -> nodes to element average
NodeToElt=sparse(mesh_fem.Nelts,mesh_fem.FEnode.n_tot);
for e=1:mesh_fem.Nelts
    NodeToElt(e,mesh_fem.ien(e,:))=1./3.;
end
...
for i=2:2;
    ...
    while ( (k<iter_max) && (cvgence~=1) )
        ...
        D_vol_Strain= sum(Dstrain(:,[ 1 2 4]),2);
        
        Source_k= -1.*(propObject.b)*(propObject.M)*D_vol_Strain; % source is -b M Deps_v
        obj_source=PDE_part_load_body('Laplacian',Config,mesh_fem,propObject,Source_k,dof_h_l);
        [Fs]=Assembly(obj_source,3);
        % 
...
% %         disp((max(abs((D_U-D_U_k)./D_U))));
    ...
    figure(3)
    plot(mesh_fem.FEnode.coor(klt_z,1),pp_o(dof_h_p.ID_array(klt_z)),'.');
%     trisurf(Ien,mesh_fem.FEnode.coor(:,1),mesh_fem.FEnode.coor(:,2), pp_o)
%         
%      % plot deformed mesh
%      
%         theU=U_o;
%         udisp = [theU(obj_elas_u.DOF_handle.ID_array(:,1)) theU(obj_elas_u.DOF_handle.ID_array(:,2))];
%         
%         figure(3)
%         plotmesh(the_coor,connect,[.2 .2 .2],'w')
%         hold on;
...
scatter3(AvgCoor(:,1),AvgCoor(:,2),Source_k)
