...
%  MODE-1 Loading
%  at t=0+
%  s_rr(r=1)=-p*,  p(r=1)=0   (Steep gradient initially induce a ct shift
%  in the solution)
%  See Detournay & Cheng 1993 for the Analytical solution

% to fix - boundary loads problems inside Elasticity block. -> temporary
% fix, no boundary loads in Elasticity Block for 1D problem.
...
propObject_u=Properties_Elastic_Isotropic(propObject.k_u,propObject.g,1.);
obj_elas_u=Elasticity_Block('Axis',mesh_fem,propObject_u,Imp_displacement,Boundary_loads,0*mySig_o);
[K_u,dof_aux]=BuildStiffness(obj_elas_u);   %% Assemble stiffness
%matrix
F_outer=2*pi*Boundary_loads(3);
F=zeros(dof_h.nrow,1);
F(dof_h.nrow)=F_outer;
U_u=K_u\F;

[Stress_u,Strain_u,AvgCoor]=Stress_And_Strain(obj_elas_u,U_u);
MeanStress=mean(Stress_u(:,1:3)')';
MeanSig_o=mean(mySig_o')';
%%%% get the corresponding pore pressure profile via Skempton coefficient
% variation of mean stress
pp_o_e=(propObject.B)*(-MeanStress); % ct on each element (linear  FE)

% analytical solution
(2./3)*(1+propObject.nu_u)*(propObject.B); % pp
-(1-2*propObject.nu_u)*coor(10)/(2*propObject.g);
...
pp_o=Elt_to_Node*pp_o_e; % initial pp field on nodes
...
for i=2:300;
    ...
    while ( (k<iter_max) && (cvgence~=1) )
        ...
        [D_U_k]=full(Solve(obj_elas));
         
        % relaxation
        D_U=beta*D_U_k+(1-beta)*D_U;
%         disp((max(abs((Dp_k_new-Dp_k)./Dp_k_new)) < eps_p));
%         
