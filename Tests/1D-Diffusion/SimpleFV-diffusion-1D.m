...
time_step= 1*h^2.; % 
n_step=100;    % nunber of steps to compute
pns=[ ];       % 
...
theta=0.5;    % theta - time integration scheme choice (theta \in [0,1] )
...
for j=1:n_step,
    ...
    %  be careful the domain is of size 2 so dimensionless time = time/ 2^2
end

%%
figure(2)
title('Pressure profile at different time '); hold on;
plot(xs,pns(1:20:n_step,:),'.-k'); hold on;
plot(xs,res(1:20:n_step,:),'-r'); hold on;
xlabel(' x');
ylabel('Pressure');
...
%%
figure(3)
title('Pressure versus time at tenth Cells '); hold on;
plot(taus,pns(:,10)) ; hold on;
plot(taus,res(:,10),'-r');
xlabel(' time');
ylabel('Pressure at 10th cell');
...
xlabel(' time');
ylabel('relative error on Pressre at 10th cell');
